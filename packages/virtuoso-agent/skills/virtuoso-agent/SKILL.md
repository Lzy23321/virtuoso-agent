...
- For a Maestro bundle, read `bundle.json`, then `maestro/maestro.ocn`, the relevant test's recorded `single.ocn`, and only when needed that test's recorded `netlist/input.scs`.
