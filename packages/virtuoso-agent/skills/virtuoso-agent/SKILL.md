...
- Use `virtuoso_inspect` with `action: "schematic"` or `action: "maestro"` for read-only design inspection. Read the compact result first, then open the JSON artifact only when details are needed.
